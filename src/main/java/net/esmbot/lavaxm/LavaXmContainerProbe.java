...
import org.helllabs.lubxmp.Module;
...
public class LavaXmContainerProbe implements MediaContainerProbe {
    ...
    public LavaXmContainerProbe(LavaXmConfig config) {
        ...
        if (defaultPan > 100 || defaultPan < 0) {
            log.error("The default pan config value is invalid, must be between 0 and 100");
            return;
        }
    }
    ...
    public MediaContainerDetectionResult probe(AudioReference reference, SeekableInputStream inputStream)
        ...
        inputStream.seek(0);
        return MediaContainerDetectionResult.supportedFormat(this, null, new AudioTrackInfo(module.getName(), "Unknow artist", info.getTotalTime(), reference.identifier, false, reference.identifier));
    }
